...
constant_minl=torch.from_numpy(np.array(minl)).data.cuda()
print(constant_maxl,constant_minl)
...
for epoch in range(EPOCH): 
    ...
    for step,(b_x,b_y,aver_l) in enumerate(train_loader): 
        ...
        output=(output.float() * (constant_maxl-constant_minl)+constant_minl) * torch.from_numpy(get_tem_mask().astype(np.float32)).cuda()
        b_y= (b_y.float()* (constant_maxl-constant_minl)+constant_minl) * torch.from_numpy(get_tem_mask().astype(np.float32)).cuda()
        aver_l=(aver_l.float() * (constant_maxl-constant_minl)+constant_minl) * torch.from_numpy(get_tem_mask().astype(np.float32)).cuda()
    ...
    if epoch>-1:
        ...
        for k,(t_x,t_y,aver_l2) in enumerate(test_loader): 
            ...
            output2.data=(output2.data.float() * (constant_maxl-constant_minl)+constant_minl) * torch.from_numpy(get_tem_mask().astype(np.float32)).cuda() #397.317
            t_y.data= (t_y.data.float()* (constant_maxl-constant_minl)+constant_minl) * torch.from_numpy(get_tem_mask().astype(np.float32)).cuda()
            aver_l2.data=(aver_l2.data.float() * (constant_maxl-constant_minl)+constant_minl) * torch.from_numpy(get_tem_mask().astype(np.float32)).cuda()
