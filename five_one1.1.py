...
testi=np.array(t[12001:,:,:,:])
trainl=np.array(ot[:12001])
...
for epoch in range(5): 
    ...
    for step,(x,y) in enumerate(train_loader): 
        ...
        print('余弦相似：',cos_sim(output.reshape(-1).detach().numpy(),b_y))
        if step % 10 == 9: 
            ...
            print('Epoch:', epoch, '|Step:', step, '|train loss:%.4f'%running_loss/10.0)
            running_loss = 0.0
...
for k,(m,n) in enumerate(test_loader): 
    ...
    output = net(t_x) 
    pre=output.reshape(-1)
    loss = loss_function(pre, t_y)
    t_loss += loss.item()
    pre_all=torch.cat([pre_all,t_y])
    print( '|loss:',t_loss/(k+1),'|余弦相似:%.4f'%cos_sim(pre.detach().numpy(),t_y)) 


# In[72]:


plt.plot(pre_all.detach().numpy(),'-k',label='预测')
