...
traini=np.array(t[:11005,:,:,:])
#validi=np.array(t[301:365,:,:,:])
testi=np.array(t[11001-delay:,:,:,:])
trainl=np.array(ot[delay:11001])
...
#        # input:size is M*3. M　is the batch　number
#        # target:size is M.
#        #pt=torch.max(input.data,1)
#        _,p = torch.max(input.data,1)
#        p1=torch.log(p.float())
#        p2=torch.log(1-p.float())
#        #p=pt[:,1]
#        loss = -self.alpha\
#               *(1-p)**self.gamma\
#               *(target\
#               *p1.long())\
#               -(1-self.alpha)*p**self.gamma*((1-target)*p2.long())
...
class FocalLoss(nn.Module):
    ...
    def __init__(self, num_class=3, alpha=None, gamma=2, balance_index=-1, smooth=None, size_average=True):
        super(FocalLoss, self).__init__()
...
for epoch in range(EPOCH): 
    ...
    for step,(x,y) in enumerate(train_loader): 
        b_x = Variable(x).cuda() 
        b_y = Variable(y).cuda()
        b_x = b_x.requires_grad_()
        b_y = b_y.requires_grad_()
        ...
        if step % 20 == 19: 
            print('Train||','Epoch:', epoch, '|Step:', step, '|train loss:%.6f:'%(running_loss/20.0),'|accuracy rate:%.6f:'%(running_correct.cpu().detach().numpy()/(20.00*BATCH_SIZE_TRAIN)))
            running_loss =0.0
            running_correct=0.0
    if epoch>EPOCH-10:
        t_loss=0.0
        ...
        print( '第%d轮结束后:'%epoch,'|average_test_loss:%.6f:'%(t_loss/(k+1)),'|accuracy rate:%.6f:'%(t_correct.cpu().detach().numpy()/((k+1)*BATCH_SIZE_TEST)))
