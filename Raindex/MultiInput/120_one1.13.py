...
for epoch in range(EPOCH): 
    ...
    for step,(x,y) in enumerate(train_loader): 
        ...
        if step % 10 == 9: 
            ...
            print('Epoch:', epoch, '|Step:', step, '|train loss:',running_loss/10.0,'|余弦相似:%.4f'%cos_sim(output.reshape(-1).cpu().detach().numpy(),b_y.cpu()))
            running_loss = 0.0
        if epoch>4*EPOCH/5:
             #t_loss = 0.0
             ...
             for k,(m,n) in enumerate(test_loader): 
                 ...
                 print( 'Test','Epoch:', epoch,'|test_loss:',loss.item(),'|余弦相似:%.4f'%cos_sim(pre.cpu().detach().numpy(),t_y.cpu()))
