...
for epoch in range(EPOCH): 
    ...
    for step,(x,y) in enumerate(train_loader): 
        ...
        if step % 10 == 9: 
            print('Train||','Epoch:', epoch, '|Step:', step, '|train loss:',running_loss/10.0,'|余弦相似:%.4f:'%cos_sim(output.reshape(-1).cpu().detach().numpy(),b_y.cpu()))
            running_loss = 0.0
        if epoch>4*EPOCH/5 and epoch%5==0:
             t_loss = 0.0
             cosim=0.0
